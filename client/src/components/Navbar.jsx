...
import { FaComments, FaCompass, FaMoon, FaSignOutAlt, FaSun } from 'react-icons/fa'
		...
		<Container maxW='container.xl'>
			<Box mb={4} py={2} borderBottom='2px' borderBottomColor={useColorModeValue('gray.100', 'gray.700')}>
				<HStack justifyContent='flex-end' maxW='container.lg' mx='auto' spacing={4}>
					...
					<Navlink to='/explore' name='Explore' icon={<FaCompass />} />
					...
					{currentUser && (<>
						<Navlink to='/chat' name='Chat' icon={<FaComments />} />
						{/* <Navlink to={`/profile/${currentUser._id}`} name='Profile' /> */}
						<Navlink to='/logout' name='Logout' icon={<FaSignOutAlt />} variant='outline' onClick={async e => { e.preventDefault(); dispatch(logout(currentUser, navigate, location)) }} />
					</>)}
